...
public class Member1DaoImpl implements Member1Dao {
	...
	public int transactionInsertUpdate() {
		...
		try {
			...
			System.out.println("Member1DaoImpl transactionInsertUpdate membner2 result-> "+result);
	...
	public int transactionInsertUpdate3() {
		...
		try {
			...
			// Transaction 관리는 transactionManager의 getTransaction을 가지고 상태따라 설정
			member1.setId("1007");
			member1.setPassword("2345");
			member1.setName("강유7");
			...
			result = session.insert("insertMember1", member2);
			System.out.println("Member1DaoImpl transactionInsertUpdate3 membner2 result-> "+result);
